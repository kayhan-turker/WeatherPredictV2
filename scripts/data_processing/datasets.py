...
def get_image_data(image_key, metadata, txn):
    ...
    if metadata == b'quality' or metadata == b'stillness':
        image_key = image_key.split(b'_')[0]
    return txn.get(image_key + b'_' + metadata).decode()
...
class LMDBDataset(Dataset):
    ...
    def get_filtered_keys(self):
        with self.env.begin() as txn:
            ...
            return [key for key, _ in txn.cursor()
                    ...
                    and (self.sun_angle is None or float(get_image_data(key, b'label', txn).split(';')[18]) >= self.sun_angle)]

    def count_sources(self):
        sources = set()
        for image_key in self.image_keys:
            sources.add(image_key.split(b'_')[0])

        print(f"Found {len(sources)} sources:")
        print(", ".join(source.decode() for source in sources))
        return len(sources)
    ...
    def get_key_pair(self, image_key_0):
        ...
        if self.pair_same_source_prob is None or self.num_sources == 1:
            ...
            same_source = image_key_0.split(b'_')[0] == image_key_1.split(b'_')[0]
        ...
        if random.random() < self.pair_same_source_prob:
            same_source_keys = [k for k in self.image_keys if k.split(b'_')[0] == image_key_0.split(b'_')[0]]
            return random.choice(same_source_keys), 1
        else:
            diff_source_keys = [k for k in self.image_keys if k.split(b'_')[0] != image_key_0.split(b'_')[0]]
            return random.choice(diff_source_keys), 0

    def __getitem__(self, idx):
        image_key_0 = self.image_keys[idx]
        label_key_0 = image_key_0 + b'_label'
        ...
        with self.env.begin(write=False) as txn:
            ...
            if self.paired:
                ...
                label_key_1 = image_key_1 + b'_label'
        ...
        return (crop_torch_image(image_0), label_0 if not self.paired else
                crop_torch_image(torch.concatenate((image_0, image_1), dim=0)),
