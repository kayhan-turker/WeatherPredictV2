...
def create_lmdb(lmdb_path, image_folder, label_folder, target_size):
    ...
    with env.begin(write=True) as txn:
        for source_id in tqdm(source_ids):
            ...
            txn.put(source_id.encode() + b'_quality', str(quality).encode())
            txn.put(source_id.encode() + b'_stillness', str(stillness).encode())
            ...
            for image_name, label in zip(sorted_images, sorted_label_records):
                ...
                key = f"{source_id}_{image_name}".encode()
                txn.put(key, image_bytes)
                txn.put(key + b'_label', label.strip().encode())
